...
for i in range(0, len(df_36)):
    ...
    #pdf.ln(1.5*th)
pdf.ln(5)
pdf.output('Annual Report.pdf', 'F')

layout = dbc.Container([
    html.Br(),
    dbc.Container([
        dcc.Location(id='urlDownloadReport', refresh=True),
        html.H3('Download Report'),
        html.Hr(),
        html.H4('Annual Report'),
        html.Div([html.Button("Download", id="btn", className='btn btn-primary btn-lg'), Download(id="download")])])])
...
def func(n_clicks):
    return send_file("Annual Report.pdf")